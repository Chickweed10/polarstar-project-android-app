...
public class GuardianMyInfoActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mProflPNReqN.setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    private void connectionCheck(){ //firebase select 조회 함수, 내 connect 테이블 조회
        Query guardianQuery = mDatabase.child("connect").child("guardian").orderByKey().equalTo(user.getUid()); //보호자 테이블 조회
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                if(myConnect.getMyCode() != null && !myConnect.getMyCode().isEmpty()){
                    if(myConnect.getCounterpartyCode() == null){ //상대방이 연결 끊었을 경우
                        if(! GuardianMyInfoActivity.this.isFinishing()){ //finish 오류 방지
